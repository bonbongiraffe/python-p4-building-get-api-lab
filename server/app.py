...
def baked_goods_list(baked_goods):
    output_list = []
    for baked_good in baked_goods:
        ...
        output_list.append(baked_good_dict)
    return output_list
...
def bakery_by_id(id):
    ...
    baked_goods = BakedGood.query.filter(BakedGood.bakery_id == id).all()
    bakery_dict = {
        "id": bakery.id,
        "name": bakery.name,
        "created_at": bakery.created_at,
        "updated_at": bakery.updated_at,
        "baked_goods": baked_goods_list(baked_goods)
    }
...
def baked_goods_by_price():
    ...
    response = make_response(jsonify(baked_goods_list(baked_goods)), 200)
    return response
